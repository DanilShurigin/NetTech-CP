...
class ConnectionHandler(threading.Thread):
        ...
        try:
            ...
            while True:
                ...
                if message.conn_id != self._id.bytes:
                    logger.info("Wrong conn_id in received message", extra={"UUID": self._id})
                    send_error_info_msg(
                        sock=self._conn_socket,
                        conn_id=self._id,
                        flags=0b00000001,
    ...
    def register_client(self, message: Message) -> None:
        ...
        try:
            ...
            logger.info("User '%s' requested registration", login, extra={"UUID": self._id})
            ...
            if get_user_database().user_exists(login):
                logger.info("User '%s' already exists in users DB", login, extra={"UUID": self._id})
                send_error_info_msg(
                    sock=self._conn_socket,
                    conn_id=self._id,
                    flags=0b00000001,
            ...
            logger.debug("Received password hash for user '%s'", login, extra={"UUID": self._id})
            ...
            logger.error("Unable to register client", extra={"UUID": self._id}, exc_info=True)
            return

    def authenticate_client(self, message: tuple[bytes, int, int, bytes]) -> None:
        """
        Провести аутентификацию клиента.

        Args:
            message (tuple[bytes, int, int, bytes]): Сообщение от клиента.
        """
        try:
            # Получаем имя пользователя из сообщения
            login = message.payload.decode()
            logger.info("User '%s' requested authentication", login, extra={"UUID": self._id})
            
            # Проверяем пользователя на наличие записи в БД.
            # Если записи нет, аутентификация запрещена без регистрации
            if not get_user_database().user_exists(login):
                logger.info("User '%s' does not exist in users DB", login, extra={"UUID": self._id})
                send_error_info_msg(
                    sock=self._conn_socket,
                    conn_id=self._id,
                    flags=0b00000001,
            ...
            if salt is None:
                logger.error("Failed to retrive SALT from users DB", extra={"UUID": self._id})
                send_error_info_msg(
                    sock=self._conn_socket,
                    conn_id=self._id,
                    flags=0b00000001,
            ...
            if stored_passwd_hash is None:
                logger.error("Failed to retrive PASSWORD from users DB", extra={"UUID": self._id})
                send_error_info_msg(
                    sock=self._conn_socket,
                    conn_id=self._id,
                    flags=0b00000001,
            ...
            logger.info("User '%s' successfully authenticated", extra={"UUID": self._id})

        except ProtocolException as err:
            logger.error("Unable to authenticate client", extra={"UUID": self._id}, exc_info=True)
            return

    def upload_file(self, message: tuple[bytes, int, int, bytes]) -> None:
        """
